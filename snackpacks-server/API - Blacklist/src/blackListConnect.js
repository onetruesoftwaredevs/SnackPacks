...
class blackListUserConnector{
    ...
    // removeBlackListedUsersByID(userID int)
    // Removes the user specified by the userID
    removeBlackListedUserByID(userID){
        return new Promise((resolve, reject) => {
            var connection = mysql.createConnection({host:this.host, user:this.user, password: this.password, port: this.port});
            connection.connect(function(err){
                if(err) reject(err);
                connection.query(`delete from snackpacks.blacklist where userID=${userID}`, function(err, result, fields){
                    if(err) reject(err);
                    connection.end(function(err){
                        if(err) reject(err);
